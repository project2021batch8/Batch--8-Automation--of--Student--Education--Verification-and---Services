...
<body bgcolor="#FFFFFF">
	...
	ResultSet rs4=stmt4.executeQuery("select * from admin");
	 ...
	 pw=rs4.getString("pass"); 
         stmt.executeUpdate("update read_data set status=0");
	 ...
	 pas=pas.substring(0,4);
      ...
      if(ss2.length>1)
      {
          ...
          //verify
              try
              {
          ResultSet rs3=stmt3.executeQuery("select * from student_data where regno='"+rno1+"'");
          if(rs3.next())
          {
...
<%
         ///remove read
                   ResultSet rs5=stmt5.executeQuery("select * from read_data");
          while(rs5.next())
          {
              if(rs5.getString("sender").equals(sender))
              {
                  if(!rs5.getString("dtime").equals(dtt))
                  {
                  //out.print("d="+rs5.getInt("id"));
                  //stmt.executeUpdate("update read_data set status=2 where id="+rs5.getInt("id")+"");
                  }
              }
         ...
         //////////////
          ...
          {
              //out.print("yes");
              stmt.executeUpdate("update read_data set status=1 where id="+rs2.getInt("id")+"");
          }
          else
          {
    ...
    //out.print("x="+x);
	 ...
	 int jj=0;
         //out.print("select * from read_data where status=1 order by id desc");
	  ResultSet rs44=stmt44.executeQuery("select * from read_data where status=1 order by id desc");
                  while(rs44.next())
				  {
					jj++;  
				  }
				  out.print("<h3 align=center>No. of Unread: "+jj+"</h3>");
				  if(jj>0)
				  {
         %>
          <table width="90%" border="0" align="center" cellpadding="5" cellspacing="0">
           <tr>
             <th width="9%" class="bor">S.No</th>
             <th width="29%" class="bor">Sender</th>
             <th width="26%" class="bor">Subject</th>
             <th width="36%" class="bor">Date / Time </th>
           </tr>
           <%
                  int ii=0;
                  ResultSet rs4=stmt4.executeQuery("select * from read_data where status=1 order by id desc");
                  while(rs4.next())
                  {
                      ii++;
                  %>
           <tr>
             <td class="bor"><%=ii%></td>
             <td class="bor"><%=rs4.getString("sender")%></td>
             <td class="bor"><%=rs4.getString("subject")%></td>
             <td class="bor"><%=rs4.getString("dtime")%></td>
           </tr>
           <%
          }
		  }//jj
